#!/usr/bin/env node
/**
 * Rebuilds data/manifest.json from data/boat-tasks.json.
 *
...
const DATA_PATH = join(ROOT, 'data', 'boat-tasks.json');
const MANIFEST_PATH = join(ROOT, 'data', 'manifest.json');

// Public Pages URL where the app fetches the data from.
const DATA_URL = 'https://support-teamam.github.io/boatstory-data/data/boat-tasks.json';
...
try {
  ...
  console.error('boat-tasks.json is not valid JSON:', e.message);
  process.exit(1);
}
